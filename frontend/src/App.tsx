...
const PRESETS = ['All missions', 'Favorites', 'Crewed', 'Broadband', 'Navigation', 'Weather', 'Science'] as const;
...
function HomePage() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() => new URLSearchParams(window.location.search).get('satellite'));
  ...
  const filtered = useMemo(() => {
    ...
    const matches = catalog.filter((satellite) => {
      ...
      return matchesSearch && (operator === 'All' || satellite.operator === operator) && (orbit === 'All' || satellite.orbit === orbit) && matchesPreset(satellite, preset, favorites);
    });
    return [...matches].sort((a, b) => sort === 'Inclination' ? b.inclination - a.inclination : sort === 'Altitude' ? a.meanMotion - b.meanMotion : a.name.localeCompare(b.name));
        ...
        <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-4">
            <div className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-3 backdrop-blur md:grid-cols-[1fr_auto_auto_auto]">
              ...
              <Filter label="Sort" value={sort} values={SORTS} onChange={(value) => setSort(value as Sort)} />
            </div>
            <div className="flex flex-wrap gap-2">{PRESETS.map((item) => <button key={item} onClick={() => setPreset(item)} className={`rounded-full border px-3 py-1.5 text-xs transition ${preset === item ? 'border-cyan-400 bg-cyan-400 text-slate-950' : 'border-slate-700 bg-slate-900 text-slate-400 hover:border-cyan-700 hover:text-cyan-300'}`}>{item}{item === 'Favorites' ? ` (${favorites.size})` : ''}</button>)}</div>
            <Scene satellites={filtered} selectedId={selectedId} onSelect={selectSatellite} time={simulationTime} observer={observer} simulationMode={speed !== 1} followSelected={followSelected} />
          ...
          <aside className="rounded-[2rem] border border-slate-800 bg-slate-900/60 p-5 backdrop-blur xl:sticky xl:top-5 xl:self-start">
            {selected ? <SatelliteDetails satellite={selected} time={simulationTime} observer={observer} favorite={favorites.has(selected.id)} following={followSelected} onFavorite={() => toggleFavorite(selected.id)} onFollow={() => setFollowSelected((value) => !value)} onClose={() => selectSatellite(null)} /> : <CatalogList satellites={filtered} favorites={favorites} onFavorite={toggleFavorite} onSelect={selectSatellite} />}
...
function CatalogList({ satellites, favorites, onFavorite, onSelect }: { satellites: Satellite[]; favorites: Set<string>; onFavorite: (id: string) => void; onSelect: (id: string) => void }) {
  const [visible, setVisible] = useState(80);
  useEffect(() => setVisible(80), [satellites]);
  return <><p className="text-xs uppercase tracking-[0.25em] text-cyan-400">Quick inspect</p><h2 className="mt-2 text-2xl font-semibold">Satellite catalog</h2><p className="mt-2 text-sm leading-6 text-slate-400">Select a marker or choose an object below to inspect its orbit.</p><div className="mt-5 max-h-[410px] space-y-2 overflow-auto pr-1">{satellites.slice(0, visible).map((satellite) => <div key={satellite.id} className="flex rounded-xl border border-slate-800 bg-slate-950/60 transition hover:border-cyan-700 hover:bg-cyan-950/20"><button onClick={() => onSelect(satellite.id)} className="flex min-w-0 flex-1 items-center justify-between p-3 text-left"><span className="min-w-0"><span className="block truncate text-sm font-medium text-slate-200">{satellite.name}</span><span className="mt-1 block text-xs text-slate-500">{satellite.operator} · NORAD {satellite.noradId}</span></span><span className="ml-2 rounded-full bg-slate-800 px-2 py-1 text-[10px] text-cyan-300">{satellite.orbit}</span></button><button aria-label={`${favorites.has(satellite.id) ? 'Remove' : 'Add'} ${satellite.name} ${favorites.has(satellite.id) ? 'from' : 'to'} favorites`} onClick={() => onFavorite(satellite.id)} className={`px-3 text-lg ${favorites.has(satellite.id) ? 'text-amber-300' : 'text-slate-600 hover:text-amber-300'}`}>★</button></div>)}{visible < satellites.length && <button onClick={() => setVisible((count) => count + 80)} className="w-full rounded-xl border border-slate-700 py-3 text-xs text-cyan-300 hover:bg-cyan-950/30">Show 80 more</button>}</div></>;
}

function SatelliteDetails({ satellite, time, observer, favorite, following, onFavorite, onFollow, onClose }: { satellite: Satellite; time: Date; observer: Observer | null; favorite: boolean; following: boolean; onFavorite: () => void; onFollow: () => void; onClose: () => void }) {
